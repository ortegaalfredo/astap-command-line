...
uses
  ...
  unit_command_line_solving, unit_command_line_general, astap_stack;
...
begin
  ...
  begin
    ...
    '-stack filename.json (Stack images specified in the json file)'
    );
  ...
  if filespecified or hasoption('stack') then
  begin
    ...
    if hasoption('stack') then
    begin
      filename2:=GetOptionValue('stack');
      Halt(stack(filename2));
    end;
